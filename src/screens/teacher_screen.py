...
from src.database.db import check_pass,check_teacher_exists,create_teacher,teacher_login,get_teacher_subjects,create_subject,get_attendance_for_teacher
...
def teacher_dashboard():
    ...
    with tab2:
        ...
        if st.button("Manage Subjects",type=type2,width="stretch", icon=":material/fact_check:"):
            st.session_state.current_teacher_tab="manage_subjects"
            st.rerun()

    with tab3:
        type3="primary" if st.session_state.current_teacher_tab=="attendance_records" else "tertiary"
        if st.button("Attendace Records", type=type3,width="stretch" ,icon=":material/ar_on_you:"):
...
def register_teacher(teacher_username,teacher_name,teacher_password,teacher_password_confirm):
    if not teacher_username or not teacher_name or not teacher_password:
        return False,"All feild required !"
...
def teacher_screen_login():
    ...
    st.space()
    st.space()
    teacher_username=st.text_input("Enter Username",placeholder="Arya Waskar")
    teacher_password=st.text_input("Enter password",type="password",placeholder="Enter Passowrd")

    st.divider()
    btn1,btn2=st.columns(2)
    with btn1:
        if st.button("Login",icon="🔑",shortcut="control+enter",width="stretch"):
...
def teacher_screen_register():
    ...
    teacher_username=st.text_input("Enter Username",placeholder="Arya")
    teacher_name=st.text_input("Enter name",placeholder="Arya Waskar")
    teacher_password=st.text_input("Enter password",type="password",placeholder="Enter Passowrd")
    teacher_password_confirm=st.text_input("Confirm your  password",type="password",placeholder="Enter Passowrd")
    ...
    with btn1:
        if st.button("Register Now",icon=":material/passkey:",shortcut="control+enter",width="stretch"):
            ...
            if success:
                st.toast("Successfully Registered! Please login.", icon="✅")
                import time
                time.sleep(1)
    ...
    with btn2:
        if st.button("Login Instead",icon=":material/passkey:",type="primary",shortcut="control+enter",width="stretch"):
            st.session_state.teacher_login_type="login"
